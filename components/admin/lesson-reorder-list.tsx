    ...
    <div className="space-y-2">
      ...
      {items.map((lesson, index) => {
        ...
        return (
          <div
            key={lesson.id}
            className="flex flex-col gap-3 rounded-xl border border-ca-ink/[0.08] bg-ca-surface p-3 transition hover:-translate-y-0.5 hover:border-ca-violet/30 hover:shadow-[0_8px_24px_rgba(20,22,58,0.08)] md:flex-row md:flex-wrap md:items-center md:gap-4"
          >
            {/* Fila 1 (móvil): reorden + miniatura + título/badges. En md: una sola línea. */}
            <div className="flex min-w-0 items-center gap-4 md:flex-1">
              <div className="flex shrink-0 flex-col gap-1">
                <Button
                  ...
                  aria-label="Subir"
                  className="grid h-11 w-11 place-items-center rounded p-0 text-ca-ink-soft hover:text-ca-violet disabled:opacity-30 md:h-9 md:w-9"
                >
            ...
            {/* Fila 2 (móvil): acciones */}
            <div className="flex shrink-0 flex-wrap items-center gap-2 md:pl-1">
              <Link
                href={`/admin/lessons/${lesson.id}`}
