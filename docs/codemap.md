...
| `lib/livekit/ventana-sala.ts` | Fase **pura** de la ventana de la sala (`antes`/`abierta`/`cerrada`) reusando las constantes −30/+120 de `access.ts`. Existe porque el gate del servidor solo dice sí/no y la portada necesita distinguir "todavía no abre" de "ya terminó" | — | 0031 |
...
| `components/classroom/live/antesala.tsx` | Antesala con la identidad de la marca (reemplaza al `PreJoin` de la librería): previa de cámara, avatar con iniciales, selector de micrófono. Las pistas se piden acá y lo elegido viaja como `LocalUserChoices`, nunca durante la conexión | — | 0031 |
| `components/classroom/live/estado-conexion.tsx` | Píldora de conexión en la barra de la sala. Va por PORTAL al hueco `#ca-sala-conexion` de `SalaShell`, que vive FUERA de `<LiveKitRoom>`; la lógica de qué decir está en `room-state.ts` | — | 0031 |
| `lib/livekit/moderation.ts` | Lógica **pura** del panel: etiquetas, confirmaciones (sacar, silenciar a todos, terminar la clase) y orden de la lista (primero quien tiene el micrófono abierto) | — | 0031 |
...
| `components/classroom/live/sala-shell.tsx` | Marco compartido de la sala (encabezado mínimo + área de video) para que el participante con cuenta y el invitado aprobado vean lo mismo; separado de la página porque la portada del invitado decide en el cliente cuándo deja de ser portada | — | 0035 |
...
| `lib/livekit/egress.ts` | Cliente twirp de Egress (`StartRoomCompositeEgress`/`Stop`/`List`) hacia el S3 de Supabase Storage, y las variables `LIVEKIT_EGRESS_ENABLED` + `SUPABASE_S3_*`. El endpoint S3 se deriva de `NEXT_PUBLIC_SUPABASE_URL` para que no puedan divergir | — | 0034 |
| `lib/livekit/egress-estado.ts` | Máquina de estados **pura** de una grabación (`starting→active→uploaded→ingesting→ready`), `filePathFor` y las etiquetas que ve el docente | — | 0034 |
| `lib/classroom/iniciar-grabacion.ts` | Arranque de la grabación compartido por sus DOS disparadores: el botón del docente y el webhook que la enciende al entrar el primer participante | — | 0034 |
...
| `components/admin/instructor-create-form.tsx` | Botón "Nueva ficha" de `/admin/docentes`. La cuenta es opcional a propósito: el relator invitado que dicta una clase suelta no usa la plataforma | en `/admin/docentes` | 0036 |
...
| `app/api/admin/modules/reorder/route.ts` | Reordena los módulos de un programa. El orden viaja como la lista COMPLETA de ids (espejo de `lessons/reorder`); una lista parcial se rechaza con 422 | `POST /api/admin/modules/reorder` | — |
| `components/admin/module-order.tsx` | Flechas de orden en cada módulo. Actúa sobre el orden que pinta el servidor, no sobre un estado local, y se desbloquea recién cuando la pantalla se puso al día | en `/admin/cohorts/[id]` | — |
| `db/migrations/0100_reorder_modules_rpc.sql` | `reorder_modules(program_id, ordered_ids)`: offset temporal + 1..N, espejo de `reorder_lessons` (0032). Rechaza listas parciales | — | — |
...
| `lib/og/brand.tsx` (`BrandCard` genérico: home y certificado) · `lib/og/photo-card.tsx` (`PhotoCard` con foto real, logo y Montserrat de `lib/og/fonts/`: landings y checkouts de Diplomado y Liderazgo) + `app/**/opengraph-image.tsx` | Tarjetas Open Graph 1200×630 (ImageResponse/Satori): genérica, checkouts Diplomado/Liderazgo, landings de Liderazgo y del Diplomado y certificado dinámico por código | `/opengraph-image`, `/pago/opengraph-image`, `/liderazgo/opengraph-image`, `/diplomado/opengraph-image`, `/verificar/[code]/opengraph-image` | — |
| `app/api/leads/route.ts` | Captura de leads del formulario de contacto y de la calculadora (`source` los distingue); un lead de `/liderazgo` dispara la invitación por WhatsApp | `POST /api/leads` | 0040 |
| `lib/whatsapp/cloud-api.ts` | Contrato mínimo con Meta Cloud API (`enviarPlantilla`) desde el número propio de la Academia; solo plantillas aprobadas | — | 0040 |
| `lib/whatsapp/invitacion-reunion-liderazgo.ts` | Invitación automática a la reunión de 15 min con la directora (plantilla `liderazgo_reunion_directora`); nunca lanza y deja fila `whatsapp` en `lead_activity` | — | 0040 |
| `app/agendar/liderazgo/route.ts` | Destino del botón de la plantilla: 302 a `LIDERAZGO_AGENDA_URL` (página de citas de Google), o a `/liderazgo` si falta | `GET /agendar/liderazgo` | 0040 |
| `scripts/whatsapp-submit-template-liderazgo.mjs` | Crea/lista la plantilla `liderazgo_reunion_directora` en el WABA | `--list` / `--submit` | 0040 |
...
| `app/api/admin/leads/tasks/[taskId]/route.ts` | Completar, reabrir o borrar una tarea (colgada de la tarea: la franja de pendientes cruza leads distintos). Al borrar una reunión cancela su evento en Google | `PATCH`/`DELETE /api/admin/leads/tasks/[taskId]` | 0038, 0039 |
| `app/api/admin/leads/[leadId]/meetings/route.ts` | Agenda una reunión real: guarda la tarea PRIMERO y agenda después, para que un fallo de Atlas no pierda lo escrito ni la muestre como agendada | `POST /api/admin/leads/[leadId]/meetings` | 0039 |
| `lib/atlas/calendario.ts` | Contrato con el endpoint de calendario de Atlas. La llave de la Service Account NO entra a este repo; el id del evento se deriva del de la tarea para que el reintento no duplique | — | 0039 |
...
| `db/migrations/0095_seed_entorno_pruebas.sql` | Siembra del Entorno de Pruebas (sandbox interno): programa SANDBOX + módulo + cohorte activa + matrícula interna; los correos automáticos de las pruebas llegan solo a los matriculados acá | — | — |
