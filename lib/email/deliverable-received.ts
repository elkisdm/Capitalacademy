// Correo de confirmación de entrega: se envía cuando un alumno sube
// exitosamente su archivo a un entregable. Reutiliza el shell/estilos de
// lib/email/deliverable-open.ts.
...
export interface DeliverableReceivedInput {
  to: string;
  studentName: string;
...
export async function sendDeliverableReceivedEmail(
  params: DeliverableReceivedInput,
): Promise<SendResult> {
  ...
  try {
    const result = await resend.emails.send({
      from: FROM_EMAIL,
      to: params.to,
      subject: `Recibimos tu entrega: ${params.deliverableTitle}`,
...
function html(d: DeliverableReceivedInput, program: string, url: string): string {
  ...
  const body = `        <tr><td style="padding:32px 32px 8px 32px;">
          ...
          <p style="margin:0 0 4px 0;font-size:15px;line-height:1.65;color:#3a3d5c;">Recibimos tu entrega en ${esc(program)}. Todo quedó guardado correctamente:</p>
        </td></tr>
        <tr><td style="padding:16px 32px 8px 32px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f9f9fb;border:1px solid #ededf0;border-radius:12px;">
            <tr><td style="padding:20px 22px;">
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                ${d.fileName ? `<tr>
                  <td style="padding:8px 0;font-size:13px;color:#9b9db5;width:130px;vertical-align:top;">Archivo</td>
                  <td style="padding:8px 0;font-size:14px;color:#14163a;font-weight:600;">${esc(d.fileName)}</td>
                </tr>` : ""}
                <tr>
                  <td style="padding:8px 0;font-size:13px;color:#9b9db5;width:130px;vertical-align:top;">Recibido</td>
                  <td style="padding:8px 0;font-size:14px;color:#14163a;font-weight:600;text-transform:capitalize;">${esc(fmtDate(d.submittedAt))} (Chile)</td>
                </tr>
              </table>
            </td></tr>
          </table>
    ...
    "\n" +
    ctaButton(url, "Ver en la plataforma");
  return shell("Entrega recibida", body);
}

function text(d: DeliverableReceivedInput, program: string, url: string): string {
  const firstName = firstNameOf(d.studentName);
  return [
    `Hola${firstName ? ", " + firstName : ""}.`,
    "",
    `Recibimos tu entrega en ${program}. Todo quedó guardado correctamente:`,
    "",
    `Entregable: ${d.deliverableTitle}`,
    ...(d.fileName ? [`Archivo: ${d.fileName}`] : []),
    `Recibido: ${fmtDate(d.submittedAt)} (Chile)`,
    "",
    `Ver en la plataforma: ${url}`,
    "",
