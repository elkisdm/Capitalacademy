// Envío one-off del correo con la encuesta ANÓNIMA de feedback post-clase de IA
// (2026-07-22, "IA aplicada al rol del asesor") a los alumnos con matrícula activa,
// segmentado POR PROGRAMA.
//
// El HTML ya está preparado en
// docs/reportes/correo-encuesta-feedback-clase-ia-2026-07-22.html (se lee desde el
// repo en runtime, no se copia aquí). Personalización: si el perfil tiene nombre,
// "Hola:" pasa a "Hola, <primer nombre>:".
//
// ---------------------------------------------------------------------------------
// ANONIMATO — REGLA CRÍTICA, NO ROMPER:
// La encuesta es anónima. El correo se envía personalizado (cada alumno recibe el
// suyo), pero el ENLACE al formulario debe ser idéntico para todos: la constante
// SURVEY_URL de abajo, sin ningún parámetro que identifique al destinatario
// (nada de ?email=, ?id=, ?uid= ni tokens por persona). Si algún día se agrega
// tracking al link, la encuesta deja de ser anónima y se rompe lo prometido en
// vivo a los alumnos. NO agregar querystrings dinámicos a SURVEY_URL ni al HTML.
// ---------------------------------------------------------------------------------
//
...
//
// Modos:
//   node scripts/send-encuesta-feedback-clase-ia.mjs --list
//     -> lista programas con matrículas activas + conteo de destinatarios. Solo lectura.
//   node scripts/send-encuesta-feedback-clase-ia.mjs --dry-run --program <slug-o-id>
//     -> imprime la lista exacta de destinatarios de ese programa. No envía.
//   node scripts/send-encuesta-feedback-clase-ia.mjs preview
//     -> envía UNA muestra real solo a PREVIEW_TO. No toca la base ni el log.
//   node scripts/send-encuesta-feedback-clase-ia.mjs send --program <slug-o-id>
...
// ---------------------------------------------------------------- config
const HTML_PATH = join(REPO, "docs/reportes/correo-encuesta-feedback-clase-ia-2026-07-22.html");
const SENT_LOG_PATH = join(__dirname, ".sent-encuesta-feedback-clase-ia-2026-07-22.json");
const SUBJECT = "¿Qué te pareció la clase de IA? (3 minutos)";
...
// URL del formulario anónimo. PLACEHOLDER — confirmar antes de enviar (ver
// docs/reportes/encuesta-feedback-clase-ia-2026-07-22.md). Un solo link, igual
// para todos los destinatarios: NO agregarle parámetros por alumno (ver bloque de
// ANONIMATO arriba).
const SURVEY_URL = "https://capitalinteligente.com/s/feedback-clase-ia-2026";
...
// Reemplaza el placeholder {{SURVEY_URL}} por la constante SURVEY_URL. El valor es
// el mismo para todos los envíos (ver bloque de ANONIMATO arriba) — no recibe
// ningún dato del destinatario.
function injectSurveyUrl(html) {
  return html.replaceAll("{{SURVEY_URL}}", SURVEY_URL);
}
...
if (MODE === "preview") {
  const html = injectSurveyUrl(personalizeHtml(readFileSync(HTML_PATH, "utf8"), "Elkis (PREVIEW)"));
...
const baseHtml = injectSurveyUrl(readFileSync(HTML_PATH, "utf8"));
